...
class AsyncNotifier<T> extends ChangeNotifier implements AsyncListenable<T> {
  ...
  void update(ResultUpdateFunction<T> updater, [WarnFunction? warnCallback]) {
    ...
    throw Exception(
      '[update] was called before initialization; make sure the [AsyncNotifier] has '
      'an initial value by passing the first value you retrieve to [set] instead of [update].',
    );
  }
